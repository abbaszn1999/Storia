/**
 * Export Tab Component for Character Vlog (Step 7)
 * 
 * Two-panel layout matching ambient mode's export-tab.tsx:
...
export function ExportTab({
    ...
    const pollStatus = async (): Promise<boolean> => {
      try {
        const statusResponse = await fetch(
          `/api/character-vlog/videos/${videoId}/export/status`,
          { credentials: 'include' }
        );
        ...
        if (!statusResponse.ok) {
          ...
          if (errData.error?.includes('not initialized')) {
            if (isMounted) {
              setError('Export was not properly initialized. Please go back to Preview and click Continue again.');
        ...
        if (data.renderStatus === 'pending' && !hasStartedRender) {
          ...
          try {
            const startResponse = await fetch(
              `/api/character-vlog/videos/${videoId}/export/start-render`,
              {
                method: 'POST',
  ...
  const handleAIMetadata = useCallback(async (platform: string) => {
    ...
    try {
      ...
      // Fetch video data to get step1Data for richer context
      if (videoId) {
        try {
          ...
          if (videoResponse.ok) {
            ...
            if (step3Data?.scenes && Array.isArray(step3Data.scenes) && step3Data.scenes.length > 0) {
              ...
              step3Data.scenes.forEach((scene: any, index: number) => {
                if (scene.title || scene.name || scene.description) {
                  parts.push(`\nScene ${index + 1}: ${scene.title || scene.name || 'Untitled'}`);
  ...
  return (
    <div className="h-full flex bg-[#0a0a0a] overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-[#FF4081]/10 rounded-full blur-[150px]" />
        <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-[#FF6B4A]/10 rounded-full blur-[120px]" />
      </div>
      ...
      <div className={cn(
        ...
        <div className="flex-shrink-0 p-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#FF4081] to-[#FF6B4A] flex items-center justify-center">
            ...
            <div>
              <div className="flex items-center gap-2">
                {renderStatus === 'done' ? (
                  ...
                  <Loader2 className="h-4 w-4 text-[#FF4081] animate-spin" />
                )}
          ...
          <div className="p-4 space-y-5 pb-40">
            ...
            <div className={cn(
              ...
              <div className="flex items-center gap-2 mb-3">
                <Video className="h-4 w-4 text-[#FF4081]" />
            ...
            <div className={cn(
              ...
              <div className="flex items-center gap-2 mb-3">
                <Monitor className="h-4 w-4 text-[#FF4081]" />
              ...
              <>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Share2 className="h-4 w-4 text-[#FF4081]" />
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        className="h-7 text-xs gap-1 text-[#FF4081] hover:text-[#FF6B4A]"
                      >
                        {isGeneratingAI ? (
                      ...
                      {selectedPlatforms.length > 0 && (
                        <Badge variant="outline" className="text-xs border-[#FF4081]/30 text-[#FF4081]">
                        ...
                        <div key={platform.id} className="space-y-2">
                          <motion.div
                            ...
                            className={cn(
                              ...
                              isDisabled
                                ...
                                : isSelected
                                  ? "bg-white/[0.08] border-[#FF4081]/50"
                          ...
                          >
                            ...
                            {!platformConnected ? (
                              ...
                              >
                                {isExpanded ? (
                                  <ChevronUp className="w-4 h-4 text-[#FF4081]" />
                                ) : (
                                  <ChevronDown className="w-4 h-4 text-[#FF4081]" />
                                )}
                              </Button>
                            )}
                           </motion.div>
                ...
                {selectedPlatforms.length > 0 && (
                  <div className="flex gap-2">
                    <Button
                      ...
                      className={cn(
                        "flex-1 bg-gradient-to-r from-[#FF4081] to-[#FF6B4A]",
                        "hover:from-[#FF5C8D] hover:to-[#FF7A5C]",
                ...
                {selectedPlatforms.length > 0 && (
                  <Button
                    ...
                    className={cn(
                      ...
                      "bg-gradient-to-r from-[#FF4081] via-[#FF5C8D] to-[#FF6B4A]",
                      "hover:from-[#FF5C8D] hover:via-[#FF6B99] hover:to-[#FF7A5C]",
                      "disabled:opacity-50 disabled:cursor-not-allowed"
                    )}
                  >
                    {isPublishing ? (
        ...
        {renderStatus !== 'done' && renderStatus !== 'failed' && (
          <div className="w-full max-w-lg text-center space-y-6">
            <motion.div
              ...
              className="relative"
            >
              <div className="w-24 h-24 mx-auto rounded-full bg-gradient-to-br from-[#FF4081]/20 to-[#FF6B4A]/20 flex items-center justify-center">
                <Loader2 className="w-12 h-12 text-[#FF4081] animate-spin" />
              </div>
            </motion.div>
            ...
            <div className="w-full bg-white/10 rounded-full h-2 overflow-hidden">
              <motion.div
                className="h-full bg-gradient-to-r from-[#FF4081] to-[#FF6B4A]"
                initial={{ width: 0 }}
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isCurrent ? "bg-[#FF4081]/20 text-[#FF4081]" :
            ...
            {error?.includes('not initialized') || error?.includes('not properly initialized') ? (
              <div className="space-y-3">
                <p className="text-xs text-white/40">
                  The export process was interrupted. Please go back to the Preview step and click "Continue" again.
                </p>
                <Button
                  variant="outline"
        ...
        {renderStatus === 'done' && exportUrl && (
          <div className="w-full h-full max-w-6xl flex flex-col items-center justify-center space-y-4 pb-24">
            ...
            >
              ...
              {/* Corner Decorations */}
              <div className="absolute top-4 left-4 w-5 h-5 border-l-2 border-t-2 border-[#FF4081]/30 rounded-tl pointer-events-none" />
              <div className="absolute top-4 right-4 w-5 h-5 border-r-2 border-t-2 border-[#FF4081]/30 rounded-tr pointer-events-none" />
              <div className="absolute bottom-4 left-4 w-5 h-5 border-l-2 border-b-2 border-[#FF4081]/30 rounded-bl pointer-events-none" />
              <div className="absolute bottom-4 right-4 w-5 h-5 border-r-2 border-b-2 border-[#FF4081]/30 rounded-br pointer-events-none" />
            </motion.div>
            ...
            >
              <Button
                ...
                className="gap-2 bg-white/[0.03] border-white/[0.08] hover:bg-white/[0.08] disabled:opacity-70"
              >
                {isDownloading ? (
                  <>
