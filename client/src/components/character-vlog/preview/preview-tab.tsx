/**
 * Preview Tab Component for Character Vlog
 * 
 * Uses custom video preview with:
...
import { ShotstackStudio, type ShotstackStudioRef } from './shotstack-studio';
...
export const PreviewTab = forwardRef<PreviewTabRef, PreviewTabProps>(({ videoId }, ref) => {
  ...
  const [referenceMode, setReferenceMode] = useState<'1F' | '2F' | 'AI'>('2F');
    ...
    const fetchStudioEdit = async () => {
      try {
        ...
        const response = await fetch(
          `/api/character-vlog/videos/${videoId}/preview/studio-edit`,
          { credentials: 'include' }
        );
        ...
        // Set reference mode from response
    ...
    return (
      <div className="flex items-center justify-center h-full min-h-[500px] bg-[#0a0a0a]">
        <div className="text-center space-y-4">
          <Loader2 className="w-10 h-10 text-[#FF4081] animate-spin mx-auto" />
      ...
      <div className="flex items-center justify-center h-full min-h-[500px] bg-[#0a0a0a]">
        <div className="text-center space-y-4 max-w-md">
          ...
          <p className="text-white/60">
            Timeline data not found. Please complete the Sound step first.
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/50">
              {formatDuration(totalDuration)} • Review your video
            </p>
          </div>
        </div>
