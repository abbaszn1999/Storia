...
interface CharacterVlogSceneBreakdownProps {
  ...
  onGeneratingChange?: (isGenerating: boolean) => void;  // Notify parent when generation starts/ends
  onNext: () => void;
}
...
export function CharacterVlogSceneBreakdown({
  ...
  useEffect(() => {
    onGeneratingChange?.(isGeneratingBreakdown);
  }, [isGeneratingBreakdown, onGeneratingChange]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (propScenes && propScenes.length > 0) {
        ...
        hasAutoGeneratedRef.current = false; // Reset so user can manually trigger
        return;
      }
  ...
  }, [propScenes?.length, sceneManifest.scenes.length, script]); // Removed isGeneratingBreakdown from deps
