/**
 * Export Tab Component (Step 6)
 * 
 * Two-panel layout for Social Commerce export:
...
const PLATFORMS = [
  { id: "youtube_shorts", name: "YouTube Shorts", icon: SiYoutube, gradient: "from-red-600 to-red-700", apiPlatform: "youtube" as LatePlatform },
  { id: "tiktok", name: "TikTok", icon: SiTiktok, gradient: "from-gray-800 to-black", apiPlatform: "tiktok" as LatePlatform },
  { id: "instagram", name: "Instagram Reels", icon: SiInstagram, gradient: "from-emerald-600 via-teal-500 to-emerald-400", apiPlatform: "instagram" as LatePlatform },
  { id: "facebook", name: "Facebook Reels", icon: SiFacebook, gradient: "from-emerald-600 to-teal-700", apiPlatform: "facebook" as LatePlatform },
];

const RENDER_STEPS = [
  { id: 'pending', label: 'Initializing', progress: 0 },
  { id: 'queued', label: 'Preparing timeline', progress: 10 },
  { id: 'fetching', label: 'Fetching media', progress: 30 },
    ...
    const pollStatus = async (): Promise<boolean> => {
      try {
        const statusResponse = await fetch(
          `/api/social-commerce/export/status/${videoId}`,
          { credentials: 'include' }
        );

        if (!statusResponse.ok) {
          const errData = await statusResponse.json();
          if (errData.error?.includes('not found')) {
            if (isMounted) {
        ...
        if (data.renderStatus === 'pending' && !hasStartedRender) {
          ...
          try {
            const startResponse = await fetch(
              `/api/social-commerce/export/start-render/${videoId}`,
              {
                method: 'POST',
            ...
            if (!startResponse.ok) {
              ...
              if (!errData.error?.includes('already')) {
                ...
                // Show error to user if Shotstack is not configured
          ...
          <div className="p-4 space-y-5 pb-40">
            ...
            <div className={cn(
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-white">Video Summary</span>
              </div>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-white/50">Title</span>
                  <span className="text-white truncate ml-4 max-w-[180px]">{videoTitle}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="text-white/50">Beats</span>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-white">Share To (Optional)</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button
                        size="sm"
                          ...
                          <motion.div
                            ...
                            className={cn(
                              ...
                              isDisabled
                                ...
                                : isSelected
                                  ? "bg-white/[0.08] border-emerald-500/50"
                            ...
                            {showMetadata && (
                              ...
                              >
                                <div className={cn(
                                  ...
                                  {['tiktok', 'instagram', 'facebook'].includes(platform.id) && (
                                    <>
                                      <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-3 pt-3">
                      ...
                      <div className={cn(
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-emerald-300">
                            Fill in metadata for selected platforms
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        className={cn(
                          "flex-1 bg-gradient-to-r from-emerald-600 to-teal-700",
                      ...
                      <Button
                        onClick={() => setPublishType(publishType === 'schedule' ? 'instant' : 'schedule')}
                        disabled={isPublishing}
                        variant="outline"
                        className={cn(
                          "border-white/10 hover:bg-white/5",
                          publishType === 'schedule' && "bg-white/10"
                        )}
                      >
        ...
        {renderStatus !== 'done' && renderStatus !== 'failed' && (
          <div className="w-full max-w-lg text-center space-y-6">
            ...
            <div className="flex justify-center gap-2 flex-wrap">
              {RENDER_STEPS.filter(s => s.id !== 'done' && s.id !== 'pending').map((step) => {
