        ...
        <div className="px-8 py-5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              {voiceOverEnabled && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-white/50 uppercase tracking-wider">Voice:</span>
                  <Popover open={voiceDropdownOpen} onOpenChange={setVoiceDropdownOpen}>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        className="h-9 min-w-[180px] justify-between bg-white/5 border-white/10 text-white hover:bg-white/10 hover:border-white/20"
                      >
                        <span className={cn("text-sm", voiceActorId ? "font-medium" : "text-white/50")}>
                          {voiceActorId 
                            ? VOICE_LIBRARY.find(v => v.id === voiceActorId)?.name || "Select voice"
                            : "Select voice"}
                        </span>
                        <ChevronDown className="h-4 w-4 shrink-0 opacity-50" />
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-[320px] p-0 bg-zinc-900 border-white/10" align="end">
                      <ScrollArea className="h-[280px]">
                        <div className="p-1">
                          {VOICE_LIBRARY.filter(v => v.language === language).map((voice) => (
                            <div
                              key={voice.id}
                              className="flex items-center gap-2 px-2 py-2.5 hover:bg-white/5 rounded-md cursor-pointer transition-colors"
                              onClick={() => {
                                if (onVoiceActorChange) {
                            ...
                            >
                              <div className="flex items-center gap-2 flex-1">
                                ...
                                <div className="flex-1">
                                  <div className={cn("text-sm text-white", voiceActorId === voice.id ? "font-medium" : "")}>
                                    {voice.name}
                                  </div>
