/**
 * Narrative Preview Tab Component
 * 
...
interface NarrativePreviewTabProps {
  videoId?: string;
}

// Expose getCurrentVolumes method for parent components
export interface NarrativePreviewTabRef {
  getCurrentVolumes: () => { master: number; sfx: number; voiceover: number; music: number } | null;
}

export const NarrativePreviewTab = forwardRef<NarrativePreviewTabRef, NarrativePreviewTabProps>(({ videoId }, ref) => {
    ...
    const fetchStudioEdit = async () => {
      try {
        ...
        const response = await fetch(
          `/api/narrative/videos/${videoId}/preview/studio-edit`,
          { credentials: 'include' }
        );
        ...
        console.log('[NarrativePreviewTab] Loaded Shotstack Edit:', {
          hasEdit: !!data.edit,
        ...
        console.error('[NarrativePreviewTab] Error fetching studio edit:', err);
  ...
  const handleEditChange = useCallback((edit: ShotstackEdit) => {
    ...
    console.log('[NarrativePreviewTab] Edit changed:', edit);
  }, []);
...
NarrativePreviewTab.displayName = 'NarrativePreviewTab';
