/**
 * Video Preview Component for Narrative Mode
 * 
 ...
 * 
 * Adapted from ambient-visual mode with pink/rose color palette.
 */
...
import { 
  ...
  SkipBack, SkipForward, Info, Maximize, Minimize
} from 'lucide-react';
...
export const ShotstackStudio = forwardRef<ShotstackStudioRef, ShotstackStudioProps>(
  function ShotstackStudio({
    ...
    useEffect(() => {
      if (initialVolumes) {
        console.log('[NarrativeShotstackStudio] Initializing volumes from saved values:', initialVolumes);
    ...
    const videoClips = useMemo(() => {
      ...
      console.log('[NarrativeShotstackStudio] Template received:', {
      ...
      console.log('[NarrativeShotstackStudio] Extracted clips:', {
        trackCount: tracks.length,
    ...
    const { sfxClips, voiceoverUrl, musicUrl } = useMemo(() => {
      ...
      console.log('[NarrativeShotstackStudio] Extracted audio tracks:', {
        trackCount: tracks.length,
    ...
    console.log('[NarrativeShotstackStudio] Current media state:', {
      currentClipIndex,
    ...
    useEffect(() => {
      ...
      console.log('[NarrativeShotstackStudio] Updating audio volumes:', {
    ...
    useEffect(() => {
      if (currentMediaUrl) {
        setIsLoading(true);
        setError(null);
        // Pause all audio when loading new media
    ...
    const handleLoadedData = useCallback(() => {
      setIsLoading(false);
      setError(null);
      // Note: For videos, audio sync/resume is handled by handlePlaying event
      // This ensures audio only starts when video actually plays, not just when loaded
    }, []);
    ...
    // Handle video waiting (buffering) - pause audio
    const handleWaiting = useCallback(() => {
      console.log('[NarrativeShotstackStudio] Video waiting/buffering - pausing audio');
      setIsBuffering(true);
      // Pause all audio when video is buffering
      sfxRefs.current.forEach(audio => audio.pause());
      if (voiceoverRef.current) voiceoverRef.current.pause();
      if (musicRef.current) musicRef.current.pause();
    }, []);

    // Handle video playing (after buffering) - resume audio synced
    const handlePlaying = useCallback(() => {
      console.log('[NarrativeShotstackStudio] Video playing - resuming audio');
      setIsBuffering(false);
      
      if (!isPlaying) return; // Don't resume audio if user paused
      ...
      const globalTime = clipStartTime + video.currentTime;
      
      // Resume audio synced to video time
      const voiceoverVol = isMuted ? 0 : volumes.voiceover * volumes.master;
      const musicVol = isMuted ? 0 : volumes.music * volumes.master;
      
      // Sync and resume voiceover
      if (voiceoverRef.current && voiceoverRef.current.src && voiceoverVol > 0) {
        voiceoverRef.current.currentTime = Math.min(globalTime, voiceoverRef.current.duration || globalTime);
        voiceoverRef.current.play().catch(() => {});
      }
      
      // Sync and resume music
      if (musicRef.current && musicRef.current.src && musicVol > 0) {
        const musicDuration = musicRef.current.duration || globalTime;
        const musicPosition = globalTime % musicDuration;
        musicRef.current.currentTime = musicPosition;
        musicRef.current.play().catch(() => {});
      }
      
      // Sync SFX
      syncAudioToTime(globalTime);
    }, [isPlaying, currentClipIndex, videoClips, isMuted, volumes, syncAudioToTime]);
    ...
    const togglePlay = useCallback(() => {
      ...
      if (isPlaying) {
        ...
        console.log('[NarrativeShotstackStudio] Playing with volumes:', {
        ...
        // For videos: let onPlaying handler start the audio after video actually starts
        // This ensures audio doesn't start before video, even during initial load
        if (video && isVideo) {
          video.play().catch(console.error);
          // Audio will be started by handlePlaying when video actually starts
        } else {
          // For images, start audio immediately
          if (voiceoverRef.current && voiceoverRef.current.src && voiceoverVol > 0) {
            voiceoverRef.current.play().catch(() => {});
          }
          if (musicRef.current && musicRef.current.src && musicVol > 0) {
    ...
    useEffect(() => {
      console.log('[NarrativeShotstackStudio] Image playback effect triggered:', {
        isImage,
        isPlaying,
        hasCurrentClip: !!currentClip,
        currentClipIndex,
      ...
      if (!isImage || !isPlaying || !currentClip) {
        console.log('[NarrativeShotstackStudio] Image playback skipped - condition not met');
        return;
      }
      
      console.log('[NarrativeShotstackStudio] Starting image playback timer for clip:', currentClipIndex);
      ...
      const interval = setInterval(() => {
        ...
        if (elapsed >= clipLength) {
          console.log('[NarrativeShotstackStudio] Clip finished, advancing...', {
            elapsed,
            clipLength,
            currentClipIndex,
            totalClips: videoClips.length,
          ...
          if (currentClipIndex < videoClips.length - 1) {
            ...
            console.log('[NarrativeShotstackStudio] Reached end of clips, stopping playback');
            setIsPlaying(false);
            setCurrentClipIndex(0);
            setCurrentTime(0);
            // Pause all audio
      ...
      return () => {
        console.log('[NarrativeShotstackStudio] Cleaning up image playback timer');
        clearInterval(interval);
      };
    ...
    const toggleFullscreen = useCallback(() => {
      const playerContainer = document.querySelector('[data-video-player]') as HTMLElement;
      if (!playerContainer) return;

      if (!isFullscreen) {
        if (playerContainer.requestFullscreen) {
          playerContainer.requestFullscreen().then(() => setIsFullscreen(true));
        }
      } else {
      ...
      return (
        <div className={cn("flex items-center justify-center bg-black/50 rounded-xl", className)} style={{ height }}>
          <div className="flex flex-col items-center gap-3 text-center p-6">
            <AlertCircle className="w-8 h-8 text-yellow-400" />
            <p className="text-white/60">No video or image shots found in timeline</p>
            <p className="text-xs text-white/40">Generate images or videos in the Storyboard step first</p>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className={cn("flex gap-4", className)} style={{ height }}>
        {/* Main Video Player */}
        <div 
          className={cn("flex-1 flex flex-col bg-black rounded-xl overflow-hidden", isFullscreen && "fixed inset-0 z-50 rounded-none")}
          data-video-player
        >
          ...
          <div className="relative flex-1 bg-black">
            ...
            {isBuffering && !isLoading && isPlaying && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-10 pointer-events-none">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-white/70 text-sm">Slow network connection</span>
                </div>
              </div>
            )}
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/80 z-10">
                <div className="text-center space-y-3">
                  ...
                  <Button
                    ...
                    className="bg-transparent border-pink-500/30 text-pink-400"
                  >
            ...
            {!isPlaying && !isLoading && !error && (
              <div
                ...
                onClick={togglePlay}
              >
                <div className="w-20 h-20 rounded-full bg-pink-500/20 backdrop-blur-sm flex items-center justify-center group-hover:bg-pink-500/30 transition-all group-hover:scale-110">
            ...
            {!isLoading && !error && (
              <div className="absolute top-4 right-4 z-20">
                <Button
                  ...
                  className="h-9 w-9 bg-black/50 hover:bg-black/70 text-white backdrop-blur-sm"
                >
                  {isFullscreen ? (
          ...
          <div className="flex-shrink-0 bg-[#0a0a0a] border-t border-white/[0.08] px-4 py-3">
            ...
            >
              ...
              {/* Progress */}
              <div 
                className="absolute inset-y-0 left-0 bg-gradient-to-r from-pink-500 to-rose-400 rounded-full pointer-events-none"
                style={{ width: `${duration > 0 ? (currentTime / duration) * 100 : 0}%` }}
              />
              {/* Handle */}
              <div 
                className="absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-white rounded-full shadow-lg transition-transform group-hover:scale-125 pointer-events-none"
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 font-mono text-sm">
                <span className="text-pink-400 font-semibold">{formatTime(currentTime)}</span>
              ...
              <div className="text-xs text-white/50 bg-white/5 px-3 py-1.5 rounded-md">
                Shot {currentClipIndex + 1} of {videoClips.length}
              </div>
            </div>
          </div>
        </div>
        ...
        {voiceoverUrl && (
          <audio 
            ...
            onLoadedData={(e) => {
              ...
              console.log('[NarrativeShotstackStudio] Voiceover audio loaded, volume set to:', vol);
            }}
          />
        )}
        {musicUrl && (
          <audio 
            ...
            onLoadedData={(e) => {
              ...
              (e.target as HTMLAudioElement).volume = vol;
              console.log('[NarrativeShotstackStudio] Music audio loaded, volume set to:', vol);
            }}
          />
        )}
      </div>
...
function TrackControl({
  ...
  const colorClasses = {
    ...
    rose: {
      bg: 'bg-rose-500/20',
      text: 'text-rose-400',
      slider: '[&_[role=slider]]:bg-rose-500 [&_.bg-primary]:bg-rose-500',
    },
  };
