...
async function editImage(
  ...
  if (runwareModelId !== model) {
    console.log(`[character-vlog:agent-4.3:image-editor] Model mapping: ${model} → ${runwareModelId}`);
  } else {
    console.warn(`[character-vlog:agent-4.3:image-editor] Model "${model}" not found in mapping, using as-is (may be AIR ID)`);
  }
  ...
  // Log for debugging
  console.log('[character-vlog:agent-4.3:image-editor] Image-to-image editing:', {
    originalImageUrl,
  ...
  console.log('[character-vlog:agent-4.3:image-editor] Calling image-to-image API:', {
    model: runwareModelId,
...
export async function editStoryboardImage(
  ...
  try {
    console.log("[character-vlog:agent-4.3:image-editor] Starting image editing:", {
      shotId,
      videoId,
      editCategory,
      model: imageModel,
    ...
    if (editCategory === "remove") {
      console.log("[character-vlog:agent-4.3:image-editor] Removal operation:", {
    ...
    console.log("[character-vlog:agent-4.3:image-editor] Image edited successfully:", {
      shotId,
    ...
    console.error("[character-vlog:agent-4.3:image-editor] Image editing failed:", error);
    return {
      editedImageUrl: "",
