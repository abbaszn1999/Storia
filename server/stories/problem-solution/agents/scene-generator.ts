...
import type { VideoModelConstraints } from "../prompts/scene-prompts";
 ...
 */
function validateInput(input: SceneGeneratorInput): void {
  ...
  if (difference > 0) {
    ...
    while (remaining > 0.5 && attempts < maxAttempts) {
      ...
      for (const scene of scenes) {
        ...
        const currentDuration = scene.duration;
        const nextSupported = supportedDurations.find(d => d > currentDuration);
...
function validateOutput(
  ...
  if (durationDiff > 0.5 && durationDiff <= 10) {
    console.warn(
      `[scene-generator] Duration mismatch detected: expected ${expectedDuration}s, got ${totalDuration}s (diff: ${durationDiff}s). Attempting to fix...`
    );
    ...
    if (fixed) {
      ...
      console.log(
        `[scene-generator] ✓ Duration fixed: ${totalDuration}s → ${newTotal}s (target: ${expectedDuration}s, final diff: ${finalDiff}s)`
      );
      
      // If still not perfect but close enough (within 2 seconds), accept it
      if (finalDiff > 1 && finalDiff <= 2) {
        console.warn(
          `[scene-generator] ⚠ Duration is close but not exact (diff: ${finalDiff}s). Accepting result.`
        );
      }
    } else {
      ...
      if (durationDiff <= 5) {
        console.warn(
          `[scene-generator] ⚠ Could not automatically fix duration, but difference is small (${durationDiff}s). Accepting result.`
        );
      } else {
        console.warn(
          `[scene-generator] ⚠ Could not automatically fix duration. Difference: ${durationDiff}s`
        );
  ...
  for (const scene of parsed.scenes) {
    ...
    if (wordCount > expectedWords + tolerance) {
      console.warn(
        `[scene-generator] Scene ${scene.sceneNumber}: Too many words (${wordCount} vs expected ~${expectedWords})`
      );
    }
  }
}
...
export async function generateScenes(
  ...
  console.log('[problem-solution:scene-generator] ═══════════════════════════════════════════════');
  console.log('[problem-solution:scene-generator] Generating scenes');
  console.log('[problem-solution:scene-generator] ═══════════════════════════════════════════════');
  console.log('[problem-solution:scene-generator] Duration:', duration, 'seconds');
  console.log('[problem-solution:scene-generator] Pacing:', pacing);
  console.log('[problem-solution:scene-generator] Target scene count:', sceneCount);
  console.log('[problem-solution:scene-generator] Story length:', storyText.length, 'characters');
  console.log('[problem-solution:scene-generator] Video model:', modelId);
  console.log('[problem-solution:scene-generator] Supported durations:', modelConstraints?.supportedDurations || 'default');
  ...
  try {
    ...
    try {
      ...
      console.error('[problem-solution:scene-generator] JSON parse error:', parseError);
    ...
    console.log('[problem-solution:scene-generator] ═══════════════════════════════════════════════');
    console.log('[problem-solution:scene-generator] Scene count:', parsed.scenes.length);
    console.log('[problem-solution:scene-generator] Total duration:', totalDuration, 'seconds');
    console.log('[problem-solution:scene-generator] Cost:', `$${response.usage?.totalCostUsd?.toFixed(4) || '0.0000'}`);
    ...
    console.error("[problem-solution:scene-generator] ═══════════════════════════════════════════════");
    console.error("[problem-solution:scene-generator] ✗ Scene generation failed");
    console.error("[problem-solution:scene-generator] ═══════════════════════════════════════════════");
    console.error("[problem-solution:scene-generator] Error:", error);
