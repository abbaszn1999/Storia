...
import {
  ...
  type SocialPlatform,
} from "../prompts/social-metadata-prompts";
import type { SocialMetadataInput, SocialMetadataOutput } from "../types";
...
export async function generateSocialMetadata(
  ...
  console.log('[social-metadata-generator] Generating metadata:', {
    platform,
    duration,
  ...
  try {
    ...
    console.log('[social-metadata-generator] Raw response:', rawOutput);
    ...
    console.log('[social-metadata-generator] Generated metadata:', {
      platform,
    ...
    console.error("[social-metadata-generator] Failed to generate metadata:", error);
...
export async function generateSocialMetadataForPlatforms(
  ...
  console.log('[social-metadata-generator] Generating metadata for multiple platforms:', platforms);
  ...
  await Promise.all(
    platforms.map(async (platform) => {
      try {
        ...
        console.error(`[social-metadata-generator] Failed for ${platform}:`, error);
