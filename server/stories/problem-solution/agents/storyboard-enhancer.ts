 ...
 */
function validateInput(input: StoryboardEnhancerInput): void {
...
export async function enhanceStoryboard(
  ...
  try {
    validateInput(input);
    console.log('[storyboard-enhancer] ✓ Input validation passed');
  } catch (error) {
    console.error('[storyboard-enhancer] ✗ Input validation failed:', error);
    throw error;
  }
  ...
  console.log('[storyboard-enhancer] Image attachments result:', {
    ...
    firstAttachment: imageAttachments[0] ? {
      ...
      hasImageUrl: imageAttachments[0].type === "input_image" ? !!imageAttachments[0].image_url : undefined,
      textPreview: imageAttachments[0].type === "input_text" ? imageAttachments[0].text.substring(0, 100) + '...' : undefined,
    } : undefined,
  });
  ...
  if (imageCount > 0) {
    ...
    console.log('[storyboard-enhancer] No images or text references - using text-only prompt');
  }
  ...
  try {
    ...
    console.log('[storyboard-enhancer] ✓ AI model response received');
    console.log('[storyboard-enhancer] Response length:', response.output.length, 'characters');
    console.log('[storyboard-enhancer] Usage:', {
    ...
    try {
      ...
      console.log('[storyboard-enhancer] ✓ JSON parsed successfully');
    } catch (parseError) {
      console.error('[storyboard-enhancer] ✗ JSON parsing failed:', parseError);
      console.error('[storyboard-enhancer] Raw response (first 500 chars):', response.output.substring(0, 500));
    ...
    try {
      ...
      console.log('[storyboard-enhancer] ✓ Output validation passed');
    } catch (validationError) {
      console.error('[storyboard-enhancer] ✗ Output validation failed:', validationError);
    ...
    console.error('[storyboard-enhancer] ═══════════════════════════════════════════════');
    console.error('[storyboard-enhancer] ✗ Storyboard enhancement failed');
    console.error('[storyboard-enhancer] ═══════════════════════════════════════════════');
    
    // Handle specific error types
    if (error.message?.includes('rate limit')) {
      console.error('[storyboard-enhancer] Rate limit error detected');
      throw new Error("Rate limit exceeded. Please try again in a moment.");
    }

    if (error.message?.includes('insufficient credits') || error.message?.includes('quota')) {
      console.error('[storyboard-enhancer] Insufficient credits/quota error detected');
      throw new Error("Insufficient credits or quota exceeded. Please check your account.");
    }

    if (error.message?.includes('JSON')) {
      console.error('[storyboard-enhancer] JSON parsing/validation error');
      throw new Error(`Invalid response format: ${error.message}`);
    }

    // Generic error
    console.error('[storyboard-enhancer] Error details:', {
