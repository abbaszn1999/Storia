...
/**
 * Create image generator function for a specific story mode
 * 
...
export async function createImageGenerator(mode: StoryMode) {
  ...
  type ImageGeneratorInput = any;
  type ImageGeneratorOutput = any;
  type ImageGenerationResult = any;
  type ImageStyle = any;
  ...
  // ═══════════════════════════════════════════════════════════════════════════
    console.log(`[${mode}:image-generator] Scene ${scene.sceneNumber} prompt details:`, {
  ...
  if (maxReferenceImages > 0) {
    ...
    if (styleReferenceUrl) {
      ...
      if (!styleReferenceUrl || typeof styleReferenceUrl !== 'string' || styleReferenceUrl.trim() === '') {
        console.error(`[${mode}:image-generator] Scene ${scene.sceneNumber} invalid style reference URL:`, styleReferenceUrl);
      } else if (!styleReferenceUrl.startsWith('http://') && !styleReferenceUrl.startsWith('https://')) {
        console.error(`[${mode}:image-generator] Scene ${scene.sceneNumber} style reference URL must be a full URL:`, styleReferenceUrl);
      } else {
        referenceImages.push(styleReferenceUrl);
        console.log(`[${mode}:image-generator] Scene ${scene.sceneNumber} using style reference (visual style modifiers disabled)`);
      }
    }
    ...
    if (characterReferenceUrl && supportsCharacterReference && referenceImages.length < maxReferenceImages) {
      ...
      if (!characterReferenceUrl || typeof characterReferenceUrl !== 'string' || characterReferenceUrl.trim() === '') {
        console.error(`[${mode}:image-generator] Scene ${scene.sceneNumber} invalid character reference URL:`, characterReferenceUrl);
      } else if (!characterReferenceUrl.startsWith('http://') && !characterReferenceUrl.startsWith('https://')) {
        console.error(`[${mode}:image-generator] Scene ${scene.sceneNumber} character reference URL must be a full URL:`, characterReferenceUrl);
      } else {
        referenceImages.push(characterReferenceUrl);
        console.log(`[${mode}:image-generator] Scene ${scene.sceneNumber} using character reference`);
      }
    } else if (characterReferenceUrl && !supportsCharacterReference) {
      console.log(`[${mode}:image-generator] Scene ${scene.sceneNumber} character reference ignored (model doesn't support it)`);
    } else if (characterReferenceUrl && referenceImages.length >= maxReferenceImages) {
      console.log(`[${mode}:image-generator] Scene ${scene.sceneNumber} character reference ignored (model limit: ${maxReferenceImages})`);
    }
    ...
    if (referenceImages.length > 0) {
      if (referenceImageFormat === 'inputs-with-tags') {
        ...
        console.log(`[${mode}:image-generator] Scene ${scene.sceneNumber} using inputs.referenceImages with tags (${referenceImages.length}/${maxReferenceImages}):`, payload.inputs.referenceImages);
      } else {
        // All other models: use inputs.referenceImages (array of URLs)
        payload.inputs = {
          referenceImages: referenceImages
        };
        console.log(`[${mode}:image-generator] Scene ${scene.sceneNumber} using inputs.referenceImages (${referenceImages.length}/${maxReferenceImages}):`, referenceImages);
      }
    } else if (styleReferenceUrl || characterReferenceUrl) {
      console.warn(`[${mode}:image-generator] Scene ${scene.sceneNumber} reference images were provided but none were valid/added`);
    }
  } else if (styleReferenceUrl || characterReferenceUrl) {
    console.log(`[${mode}:image-generator] Scene ${scene.sceneNumber} reference images ignored (model doesn't support them)`);
  }
  ...
  const payloads = items.map((item) => {
    ...
    console.log(`[${mode}:image-generator:batch] Processing batch of ${items.length} images...`);
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    console.log(`[${mode}:image-generator:batch] Raw AI response received:`, {
    ...
    console.log(`[${mode}:image-generator:batch] Results mapped: ${successCount} success, ${failCount} failed, ${resultByTaskUUID.size} total`);
    ...
    return items.map((item) => {
      ...
      if (!sceneInfo) {
        console.error(`[${mode}:image-generator:batch] No scene mapping for taskUUID: ${taskUUID}`);
        return {
          sceneId: item.scene.id,
          sceneNumber: item.scene.sceneNumber,
          imageUrl: "",
      ...
      if (imageData?.imageURL) {
        console.log(`[${mode}:image-generator:batch] Scene ${sceneInfo.sceneNumber} completed ✓ (taskUUID: ${taskUUID.substring(0, 8)}...)`);
        return {
        ...
        console.error(`[${mode}:image-generator:batch] Scene ${sceneInfo.sceneNumber} failed:`, errorMsg);
        return {
          sceneId: sceneInfo.sceneId,
          sceneNumber: sceneInfo.sceneNumber,
          imageUrl: "",
    ...
    console.error(`[${mode}:image-generator:batch] Batch failed:`, errorMessage);
    ...
    if (isTransientError) {
      console.warn(`[${mode}:image-generator:batch] Transient error detected, retrying in 5 seconds...`);
      await sleep(5000);
      
      try {
        console.log(`[${mode}:image-generator:batch] Retry attempt starting...`);
        ...
        const retryResults = items.map((item) => {
          ...
          if (imageData?.imageURL) {
            console.log(`[${mode}:image-generator:batch] Retry: Scene ${sceneInfo.sceneNumber} completed ✓`);
            return {
              sceneId: sceneInfo.sceneId,
              sceneNumber: sceneInfo.sceneNumber,
              imageUrl: imageData.imageURL,
            ...
            console.error(`[${mode}:image-generator:batch] Retry: Scene ${sceneInfo.sceneNumber} failed:`, retryError);
            return {
              sceneId: sceneInfo.sceneId,
              sceneNumber: sceneInfo.sceneNumber,
              imageUrl: "",
        ...
        console.log(`[${mode}:image-generator:batch] Retry complete: ${retrySuccessCount}/${retryResults.length} succeeded`);
  ...
  for (const failedResult of failedResults) {
    ...
    console.log(`[${mode}:image-generator:retry] Retrying Scene ${failedResult.sceneNumber}...`);
    ...
    try {
      ...
      if (data?.imageURL) {
        console.log(`[${mode}:image-generator:retry] Scene ${failedResult.sceneNumber} retry succeeded ✓`);
        retriedResults.push({
        ...
        console.log(`[${mode}:image-generator:retry] Scene ${failedResult.sceneNumber} retry failed again`);
        retriedResults.push({
          ...failedResult,
      ...
      console.error(`[${mode}:image-generator:retry] Scene ${failedResult.sceneNumber} retry error:`, errorMessage);
      retriedResults.push({
        ...failedResult,
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // DIMENSION SELECTION: Use video dimensions if model requires matching
  // Some video models (like OpenAI Sora 2 Pro) require input image dimensions
  // to exactly match output video dimensions
  // ═══════════════════════════════════════════════════════════════════════════
  let dimensions: { width: number; height: number };
  
  if (videoModel && videoResolution && requiresMatchingDimensions(videoModel)) {
    // Video model requires matching dimensions - use video dimensions
    const videoDims = getVideoDimensionsForImageGeneration(videoModel, aspectRatio, videoResolution);
    
    if (videoDims) {
      dimensions = videoDims;
      console.log(
        `[${mode}:image-generator] Using video dimensions for matching: ` +
        `${dimensions.width}x${dimensions.height} (video model: ${videoModel}, resolution: ${videoResolution})`
      );
    } else {
      // Fallback to standard image dimensions if video dimensions not found
      console.warn(
        `[${mode}:image-generator] Video model ${videoModel} requires matching dimensions but dimensions not found. ` +
        `Falling back to standard image dimensions.`
      );
      dimensions = getImageDimensions(aspectRatio, imageResolution, imageModel);
    }
  } else {
    // Standard case: use image resolution
    dimensions = getImageDimensions(aspectRatio, imageResolution, imageModel);
  }
  ...
  const batchItems: BatchItem[] = scenes.map((scene: any) => {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    console.log(`[${mode}:image-generator] Processing batch ${i + 1}/${batches.length} (${batch.length} images)`);
  ...
  const errors = allResults
    ...
    console.log(`[${mode}:image-generator] Generation complete:`, {
    total: scenes.length,
  ...
  const output: ImageGeneratorOutput = {
    ...
    console.log(`[${mode}:image-generator] Returning output with`, output.scenes.length, "scenes");
...
/**
 * Generate images (backward compatibility - uses problem-solution mode by default)
 */
