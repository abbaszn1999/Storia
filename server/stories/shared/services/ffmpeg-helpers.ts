 ...
 * Case 2 (Video > Audio): Video Speedup First → Dual Adjust → TRIM Video
 */
const SPEED_LIMITS = {
  video: { 
...
export interface SyncResult {
  ...
  videoSpeed: number;
  audioSpeed: number;
  method: 'none' | 'video-slowdown' | 'video-speedup' | 'audio-slowdown' | 'dual-adjust' | 'loop' | 'trim';
 ...
 *   Priority 1: Speed up video only (1.0x - 1.25x)
 *   Priority 2: Max video speedup + slow down audio (dual adjust)
...
export function calculateDualSpeedSync(
  ...
  if (videoDuration < audioDuration) {
    ...
    // NEW PRIORITY: Video Speed First → Audio Adjust → TRIM Video
    // ═══════════════════════════════════════════════════════════════
    
    const ratio = audioDuration / videoDuration;
    
    // Priority 1: Try video speedup only (prefer video adjustment first)
    // Calculate required video speed to match audio
    const requiredVideoSpeed = videoDuration / audioDuration;
    
    if (requiredVideoSpeed >= 1.0 && requiredVideoSpeed <= SPEED_LIMITS.video.max) {
      // Video can be sped up to match audio (within limits)
      return {
        targetDuration: audioDuration,
        videoSpeed: requiredVideoSpeed,
        audioSpeed: 1.0,
